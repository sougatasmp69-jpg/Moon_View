/**
 * Reusable Drag Controls & Three.js Scenes for MOON_VIEW
 * Available as ES Module and global window.__MOON_VIEW_SCENES__
 */
...
export function addDragControls(target, domElement, options = {}) {
  ...
  // Drag Hint Tooltip (persisted in localStorage)
  let hintEl = null;
  ...
  const onPointerMove = (clientX, clientY) => {
    ...
    // 1:1 Grabbing Motion:
    // Drag left (deltaX < 0) -> rotates left (target.rotation.y decreases)
    // Drag right (deltaX > 0) -> rotates right (target.rotation.y increases)
    // Drag up (deltaY < 0) -> tilts up (target.rotation.x decreases)
    // Drag down (deltaY > 0) -> tilts down (target.rotation.x increases)
